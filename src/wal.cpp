...
WriteAheadLog::WriteAheadLog(const std::string &path) : path_(path) {
    ...
    log_file_.open(path_, std::ios::app | std::ios::binary);
    if (!log_file_) {
...
void WriteAheadLog::append(Operation op, const std::string &key, const std::string &value, uint64_t seqNumber) {
    ...
    if (buffer_size_ + entry_size > MAX_BUFFER_SIZE) {
        flushBuffer();
    }
...
void WriteAheadLog::flushBuffer() {
    if (buffer_size_ == 0 || !log_file_.is_open()) {
        return;
    }

    log_file_.write(buffer_.data(), buffer_size_);
    log_file_.flush();
