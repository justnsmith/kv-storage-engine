...
class WriteAheadLogTest {
  public:
    explicit WriteAheadLogTest(const std::string &path) : path_(path), wal_(path) {
    }
...
bool test_multiple_records_over_preserved(WriteAheadLogTest &fixture) {
    ...
    ASSERT_EQ(static_cast<int>(std::get<0>(applied[2])), static_cast<int>(Operation::DELETE), "Third op should be DELETE");
...
bool test_corruption_stops_replay(WriteAheadLogTest &fixture) {
    ...
    ASSERT_TRUE(file.is_open(), "Failed to open WAL file");
